...
db.collection => load(callback) // used by db.connect()
db.collection => save(callback) // used by db.disconnect()
db.collection ===> createIndex(key) // key is a string of field/property name to index on
